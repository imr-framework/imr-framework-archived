...
Version 1.3 added the variables in the middle layers for better visualization during training
...
x_train = abs(image2[:,:,0:200])
y_train = abs(GTimage[:,:,0:200])
x_test = abs(image2[:,:,200:])
y_test = abs(GTimage[:,:,200:])
...
print('optimizing')
optimize(num_iterations=1) # very slow on cpu

#save the model

# tf.saved_model.simple_save(sess,
#                            export_dir="Unetmodel2/",
#                            inputs={"x": x, "y": y_true},
#                            outputs={"z": ypred})
...
def getActivations(layer,xx,yy,slice):
    ...
    plt.figure(1, figsize=(20, 20))
    plt.title('Filter '+ str(layer.name) + str(slice))
    plt.imshow(units[0, :, :, slice], interpolation="nearest", cmap="gray")
...
getActivations(final_layer,xx,yy,32)
